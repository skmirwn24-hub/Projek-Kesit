...
-- Berdasarkan panduan resmi Supabase Postgres Best Practices
-- ========================================================
...
-- ========================================================
-- KESIT Management - Fitur Absensi Siswa (Migration 003)
-- ========================================================
...
BEGIN
  ...
  IF v_existing_id IS NOT NULL THEN
    ...
    IF v_existing_hadir != p_status_hadir AND p_paket_siswa_id IS NOT NULL THEN
      IF p_status_hadir = 'Hadir' THEN
        UPDATE public.paket_siswa SET kuota_terpakai = GREATEST(0, kuota_terpakai + 1) WHERE id = p_paket_siswa_id;
      ELSE
        UPDATE public.paket_siswa SET kuota_terpakai = GREATEST(0, kuota_terpakai - 1) WHERE id = p_paket_siswa_id;
      END IF;
    END IF;
  ELSE
    INSERT INTO public.absensi_siswa (
      siswa_id, paket_siswa_id, pelatih_id, tanggal, nomor_sesi, kategori, status_hadir, catatan, dicatat_oleh
    ) VALUES (
      p_siswa_id, p_paket_siswa_id, p_pelatih_id, p_tanggal, p_nomor_sesi, p_kategori, p_status_hadir, p_catatan, p_dicatat_oleh
    ) RETURNING id INTO v_absensi_id;
    IF p_status_hadir = 'Hadir' AND p_paket_siswa_id IS NOT NULL THEN
      UPDATE public.paket_siswa SET kuota_terpakai = kuota_terpakai + 1 WHERE id = p_paket_siswa_id;
    END IF;
  END IF;
...
DECLARE
  v_rec RECORD;
BEGIN
  SELECT siswa_id, paket_siswa_id, status_hadir INTO v_rec
  FROM public.absensi_siswa WHERE id = p_absensi_id;
  IF NOT FOUND THEN
  ...
  IF v_rec.status_hadir = 'Hadir' AND v_rec.paket_siswa_id IS NOT NULL THEN
    UPDATE public.paket_siswa SET kuota_terpakai = GREATEST(0, kuota_terpakai - 1) WHERE id = v_rec.paket_siswa_id;
  END IF;
...
SELECT
  ab.id, ab.siswa_id, s.id_siswa, s.nama_lengkap, s.nama_panggilan,
  s.pelatih_pemilik_id, pp.nama AS pelatih_pemilik,
  ab.pelatih_id, pl.nama AS nama_pelatih_mengajar,
  ab.tanggal, ab.nomor_sesi, ab.kategori, ab.status_hadir, ab.catatan,
  ab.paket_siswa_id, ps.kuota_total, ps.kuota_terpakai,
  ab.dicatat_oleh, ab.created_at
